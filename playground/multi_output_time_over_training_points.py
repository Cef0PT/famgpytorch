...
DATA_FILE_PATH = Path(DIRECTORY_PATH, "multi_time_over_sample_nb_cpu.csv")
...
class ConventionalGPModelLinearCG(BaseKernel):
    def __init__(self, train_inputs, train_targets, likelihood):
...
class ApproxGPModel10(BaseKernel):
    def __init__(self, train_inputs, train_targets, likelihood):
        ...
        self.covar_module = famgpytorch.kernels.MultitaskKernelLinearCG(
            famgpytorch.kernels.RBFKernelApprox(number_of_eigenvalues=10), num_tasks=2, rank=1
        )
...
class ApproxGPModel20(BaseKernel):
    def __init__(self, train_inputs, train_targets, likelihood):
        ...
        self.covar_module = famgpytorch.kernels.MultitaskKernelLinearCG(
            famgpytorch.kernels.RBFKernelApprox(number_of_eigenvalues=20), num_tasks=2, rank=1
        )
